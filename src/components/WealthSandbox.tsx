...
export const WealthSandbox = ({
  ...
  const annualBonus = isOptimized ? 0.02 : 0;
  const effectiveReturn = liveReturn + annualBonus;
  const realRate = (1 + effectiveReturn) / (1 + inflationRate) - 1;

  const annualContribution = monthlyContribution * 12;
  const myGoal = 500000; // You can change this or pass it as a prop

  // We'll keep an unrounded balance copy just for accurate goal detection
  let unroundedCurrent = initialAmount;

  for (let i = 0; i <= years; i++) {
    // Apply crash FIRST (so year 10 shows the damaged value)
    let isCrashYear = isPanicMode && i === 10;
    if (isCrashYear) {
      currentBalance *= 0.70;           // 30% drop
      unroundedCurrent *= 0.70;
    }

    let currentVal = Math.round(currentBalance);
    let delayVal = i < 5 ? Math.round(initialAmount) : Math.round(delayBalance);
    ...
    // Grow for next year (only if not the final point)
    if (i < years) {
      currentBalance = currentBalance * (1 + realRate) + annualContribution;
      unroundedCurrent = unroundedCurrent * (1 + realRate) + annualContribution;

      if (i >= 5) {
        delayBalance = delayBalance * (1 + realRate) + annualContribution;
      }
    }
  }
  ...
  // Goal year using unrounded balance → more precise
  const goalYear = (() => {
    let bal = initialAmount;
    for (let y = 0; y <= years; y++) {
      if (bal >= myGoal) return y;
      if (y < years) {
        bal = bal * (1 + realRate) + annualContribution;
      }
    }
    return undefined;
  })();
    ...
    <div className="p-0 overflow-hidden border-2 border-slate-200 rounded-3xl bg-white shadow-2xl w-full my-6 font-sans">
      ...
      <div className="p-6 bg-slate-900 text-white">
        ...
        <p className="text-slate-400 text-xs mt-1 italic">
          Expected wealth after {years} years, adjusted for {(inflationRate * 100).toFixed(1)}% inflation.
        </p>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-900 font-medium">
              Waiting 5 years deletes <span className="font-bold">${costOfDelay.toLocaleString()}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="h-56 w-full p-4 relative">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 20, right: 30, left: 40, bottom: 40 }}>
            ...
            <XAxis
              ...
              tickLine={false}
              axisLine={false}
              label={{ value: 'Years from now', position: 'insideBottom', offset: -25, style: { fill: '#64748b', fontWeight: 600 } }}
            />
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              label={{ value: 'Total Wealth', angle: -90, position: 'insideLeft', offset: 0, style: { fill: '#64748b', fontWeight: 600, textAnchor: 'middle' } }}
            />
            <Tooltip
            ...
            {/* THE RESILIENCE DOT – only if crash year exists in data */}
            {isPanicMode && years >= 10 && data[10] && (
              <ReferenceDot
                x={10}
                y={data[10].balance}
                r={6}
      ...
      <div className="p-6 bg-slate-50 space-y-6">
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-bold text-slate-500">
            ...
            <span className="text-blue-600">{(effectiveReturn * 100).toFixed(1)}%</span>
          </div>
          <input
            type="range"
            min="0"
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-medium mt-1">
                To reach your goal 2 years faster, increase your monthly contribution by just $200.
              </p>
            </div>
            <button
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm font-medium">
              {liveReturn < 0.08
                ? `Increasing optimism to 8% could add ~$${(finalTotal * 0.2).toLocaleString()} more.`
